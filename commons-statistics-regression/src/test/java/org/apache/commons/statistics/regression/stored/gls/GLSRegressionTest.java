package org.apache.commons.statistics.regression.stored.gls;
...
class GLSRegressionTest extends AbstractRegressionTest {
    ...
    protected AbstractRegression createRegression(RegressionDataLoader myData) {
        GLSRegression regression = new GLSRegression(myData.getInputData());
        return regression;
    }
    ...
    public void notEnoughData() {
        double[] reducedY = new double[yData.length - 1];
        double[][] reducedX = new double[xData.length - 1][];
        double[][] reducedO = new double[omega.length - 1][];
        System.arraycopy(yData, 0, reducedY, 0, reducedY.length);
        System.arraycopy(xData, 0, reducedX, 0, reducedX.length);
    ...
    /**
     * Verifies that setting X, Y and covariance separately has the same effect as
     * newSample(X,Y,cov).
     */
    @Test
    public void testNewSample2() {
        ...
        GLSRegression regression = new GLSRegression();
    ...
    /**
     * Verifies that GLS with identity covariance matrix gives the same results as
     * OLS.
     */
    @Test
    public void testGLSOLSConsistency() {
        RealMatrix identityCov = MatrixUtils.createRealIdentityMatrix(16);
        GLSRegression glsModel = new GLSRegression();
        ...
        // TODO: Should have assertRelativelyEquals(double[], double[], eps) in
        // TestUtils
    ...
    /**
     * Generate an error covariance matrix and sample data representing models with
     * this error structure. Then verify that GLS estimated coefficients, on
     * average, perform better than OLS.
     */
    @Test
    public void testGLSEfficiency() {
        ...
        GLSRegression gls = new GLSRegression();
