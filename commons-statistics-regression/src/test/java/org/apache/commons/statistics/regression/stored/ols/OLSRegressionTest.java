...
public class OLSRegressionTest extends AbstractRegressionTest {
    ...
    @Override
    public void setUp() {
        yData = new double[] {11.0, 12.0, 13.0, 14.0, 15.0, 16.0};
        xData = new double[6][];
        xData[0] = new double[] {0, 0, 0, 0, 0};
        xData[1] = new double[] {2.0, 0, 0, 0, 0};
        xData[2] = new double[] {0, 3.0, 0, 0, 0};
        xData[3] = new double[] {0, 0, 4.0, 0, 0};
        xData[4] = new double[] {0, 0, 0, 5.0, 0};
    ...
    public void testHat() {
        ...
        /*
         * This example is from "The Hat Matrix in Regression and ANOVA", David C.
         * Hoaglin and Roy E. Welsch, The American Statistician, Vol. 32, No. 1 (Feb.,
         * 1978), pp. 17-22.
         *
         */
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = i; j < 10; j++) {
                Assertions.assertEquals(referenceData[k], hat.get(i, j), 10e-3);
                Assertions.assertEquals(hat.get(i, j), hat.get(j, i), 10e-12);
                k++;
            }
        }

        /*
         * Verify that residuals computed using the hat matrix are close to what we get
         * from direct computation, i.e. r = (I - H) y
         */
        double[] residuals = model.estimateResiduals();
        StatisticsMatrix identity = StatisticsMatrix.identity(10);
        double[] hatResiduals = identity.minus(hat).mult(model.getY()).toArray1D();
        Assertions.assertArrayEquals(residuals, hatResiduals, 10e-12);
    }

    /**
     * Test Longley dataset against certified values provided by NIST. Data Source:
     * J. Longley (1967) "An Appraisal of Least Squares Programs for the Electronic
     * Computer from the Point of View of the User" Journal of the American
    ...
    public void testLongly() {
        ...
        double[] betaHat = model.estimateBeta();
        Assertions.assertArrayEquals(betaHat, new double[] {-3482258.63459582, 15.0618722713733, -0.358191792925910E-01,
            -2.02022980381683, -1.03322686717359, -0.511041056535807E-01, 1829.15146461355}, 2E-8);

        // Check expected residuals from R
        double[] residuals = model.estimateResiduals();
        Assertions.assertArrayEquals(residuals,
            new double[] {267.340029759711, -94.0139423988359, 46.28716775752924, -410.114621930906, 309.7145907602313,
                -249.3112153297231, -164.0489563956039, -13.18035686637081, 14.30477260005235, 455.394094551857,
                -17.26892711483297, -39.0550425226967, -155.5499735953195, -85.6713080421283, 341.9315139607727,
                -206.7578251937366},
            1E-8);

        // Check standard errors from NIST
        double[] errors = model.estimateBetaStandardErrors();
        Assertions.assertArrayEquals(new double[] {890420.383607373, 84.9149257747669, 0.334910077722432E-01,
        ...
        betaHat = model.estimateBeta();
        Assertions.assertArrayEquals(betaHat, new double[] {-52.99357013868291, 0.07107319907358, -0.42346585566399,
            -0.57256866841929, -0.41420358884978, 48.41786562001326}, 1E-11);

        // Check standard errors from R
        errors = model.estimateBetaStandardErrors();
        Assertions.assertArrayEquals(new double[] {129.54486693117232, 0.03016640003786, 0.41773654056612,
            0.27899087467676, 0.32128496193363, 17.68948737819961}, errors, 1E-11);

        // Check expected residuals from R
        residuals = model.estimateResiduals();
        Assertions.assertArrayEquals(residuals,
            new double[] {279.90274927293092, -130.32465380836874, 90.73228661967445, -401.31252201634948,
                -440.46768772620027, -543.54512853774793, 201.32111639536299, 215.90889365977932, 73.09368242049943,
                913.21694494481869, 424.82484953610174, -8.56475876776709, -361.32974610842876, 27.34560497213464,
                151.28955976355002, -492.49937355336846},
            1E-10);
    ...
    public void testNewSampleDataXNull() {
        Assertions.assertThrows(IllegalArgumentException.class,
            () -> myData.inputNewSampleData(new double[] {}, null));
    }
    ...
    public void testNoDataNPECalculateBeta() {
        OLSRegression model = new OLSRegression(myData.getInputData());
        myData.clearData();
        Assertions.assertThrows(NullPointerException.class, () -> model.estimateBeta());
    }
    ...
    public void testPerfectFit() {
        myData.inputNewSampleData(yData, xData);
        double[] betaHat = regression.estimateBeta();
        ...
        StatisticsMatrix errors = new StatisticsMatrix(new DMatrixRMaj(regression.estimateBetaVariance()));
    ...
    public void testWampler1() {
        ...
        double[] se = model.estimateBetaStandardErrors();
        ...
        Assertions.assertEquals(0, model.calculateRegressionErrorVariance(), 1.0e-7);
    ...
    public void testWampler3() {
        ...
        double[] se = model.estimateBetaStandardErrors();
        Assertions.assertArrayEquals(se, new double[] {2152.32624678170, 2363.55173469681, 779.343524331583,
        ...
        Assertions.assertEquals(5570284.53333333, model.calculateRegressionErrorVariance(), 1.0e-6);
    ...
    public void testWampler4() {
        ...
        Assertions.assertEquals(55702845333.3333, model.calculateRegressionErrorVariance(), 1.0e-4);
    ...
    public void testYVariance() {
        setUp();
        OLSRegression model = new OLSRegression(myData.getInputData());
        Assertions.assertEquals(model.estimateRegressandVariance(), 3.5, 0);
    }
